...
int main() {
  ...
  string filename;
  if (pT200) {
      filename = "mymain_b_meson_baryon_tagger - bbbar_creation";
  } else if (pT45) {
      filename = "mymain_b_meson_baryon_tagger - bbbar_creation_low_energy";
  }

  // myfile.open(filename);
  // myalphafile.open("Alpha_Development_b_quark");
  myfile.open(filename, ios::out);
  myalphafile.open("Alpha_Development_b_quark", ios::out);


  for (int runNumber = 1; runNumber <= 5; ++runNumber) {
    string runFolder = "run_0" + to_string(runNumber);
    string lheFile;

    if (pT200) {
        lheFile = "/Users/Edo/Simulations/MG5_aMC_v3_5_1/bbbar_process/Events/" + runFolder + "/unweighted_events.lhe";
    } else if (pT45) {
        lheFile = "/Users/Edo/Simulations/MG5_aMC_v3_5_1/bbbar_process_low_energy/Events/" + runFolder + "/unweighted_events.lhe";
    }
    ...
    pythia.readString("Next:numberShowProcess = 0"); //Print event process record n times
    pythia.readString("Init:showChangedParticleData = 0"); //Print a list of particle and decay data for those particles that were changed (one way or another).
    ...
    for(int iEvent = 0; iEvent < nEvent; ++iEvent) {
      ...
      // Initialize counter for b mesons and baryons in the current event
      int bParticleCount = 0;
      ...
      for (int i = 0; i < pythia.event.size(); ++i) { // Loop over all particles in the evnet 
        ...
        // if (pythia.event[i].idAbs() == 3122) {
            ...
            // Generates a random number between 0 and 1 for each Lambda particle and includes the particle only if the random number is less than 0.4.
        ...
        // Here we would like to see if there was a B meson or B baryon insde the jets as a intermediate particle.
        bool isBParticle = false;
        if (pythia.event[i].idAbs() == 511 || pythia.event[i].idAbs() == 521 || pythia.event[i].idAbs() == 531 || pythia.event[i].idAbs() == 5122) {
          isBParticle = true;
          bParticleCount++; // Increment the counter for b mesons/baryons
        }
        
        if (!pythia.event[i].isFinal() && !isBParticle) continue; // Accept only final state particles and B particles
        ...
        if (isBParticle) { // Reduce the pT and energy of B particles to avoid double-counting
          //particle.reset_PtYPhiM(particle.pt()/100., particle.rapidity(), particle.phi(), particle.m());
          particle.reset_momentum(particle.px()/10000., particle.py()/10000., particle.pz()/10000., particle.e()/10000.);
        }
      ...
      cout << "Event " << iEvent << " contains " << bParticleCount << " b mesons/baryons." << endl;
    ...
    //   vector<PseudoJet> inclusive_jets= sorted_by_pt(clust_seq.inclusive_jets(ptmin)); // Use it if you would like to use pT min (cancel the following line)
      ...
      for (unsigned int i = 0; i < inclusive_jets.size() ; i++) {
        ...
        cout << "jet " << i << ": "<< "pT:" << inclusive_jets[i].pt() <<"   "<< "eta:" << inclusive_jets[i].eta() <<"   "<< "phi:" << inclusive_jets[i].phi() << "     Number of constituents: " << constituents.size() << endl; 
        ...
        // b_id is a variable that will get the value 0 if the there is a b-meson inside the jet or 1 if there is a b-baryon inside the jet.
        int b_id = -1;
        bool mesonFound = false;
        bool baryonFound = false;
        
        
        
        bool skipJet = false;
        for (unsigned j = 0; j < constituents.size(); j++) {
            double b_random_number = ((double) rand() / (RAND_MAX));
            int index = constituents[j].user_index();
            int id = event[index].id();
            if (abs(id) == 511 || abs(id) == 521 || abs(id) == 531) {
                if (b_random_number < 0.8) { // With a 80% chance, decide to skip this jet
                    skipJet = true;
                    break; // Found a b meson and decided to skip, no need to check further
                }
            }   
        }
        // If decided to skip this jet, continue to the next one
        if (skipJet) {
            continue;
        }
        ...
        for (unsigned j = 0; j < constituents.size(); j++) {
          ...
          // Early check for b-mesons or b-baryons
          if (abs(id) == 5122 || abs(id) == 511 || abs(id) == 521 || abs(id) == 531) {
              if (abs(id) == 5122) {
                  baryonFound = true;
              } else {
                  mesonFound = true;
              }
              // Skip further analysis for this particle
              continue; // This will skip the current iteration and not include this particle in further analysis
          }
        ...
        for (auto& particle : othersFinalConstituents) {
            ...
            // If the particle is a hadron and is neutral and if the absolute id is 3122 and if the decay distance is greater or equal to 500
            // if (particle.isHadron && particle.isNeutral && abs(particle.id) == 3122 && particle.decayDistance >= 500) {
      ...
      //   }
        ...
        // int photon, electron, muon, hadron, hadron_0, lambda;
        ...
        for (int j = 0; j < finalConstituents.size(); j++) {
            ...
            // int photon = 0, electron = 0, muon = 0, hadron = 0, hadron_0 = 0, lambda = 0;
          ...
          //   }
            ...
            // myfile << "     " << photon << "     " << electron << "     " << muon << "     " << hadron << "     " << hadron_0 << "     " << lambda <<  "     "  << pt_rel << "     " << r << "     " << alpha << "     ";
        ...
        // Determine the label based on whether a jet had a meson or a baryon inside
        if (mesonFound) {
          b_id = 0;
        } else if (baryonFound) {
          b_id = 1;
        ...
        myfile << b_id << "     " << endl; //Store 0,1 or 2 whether it was a ddbar process, ssbar process or none of them found in the jet.
